...
probstruchandle = str2func(strcat(problem, 'Structure'));
[~, ~, ~, ~,VarBds,~,~, ~, ~ , ~, ~] = probstruchandle(1, solverseed);
...
function [ncalls, xnew, xnewfn, xnewFnVar, xnewconstraint, ...
    ...
    ConstraintCov, mk, iseed, logfid, minmax,VarBds)
...
for i=1:id
    ...
    xoldfn = 0;
    xold(i)=xold(i)+1;
    if(xold(i)< VarBds(i,1)||xold(i)> VarBds(i,2))
        xoldfn = nan;
    else
...
if(sum(ixquad< VarBds(:,1))||sum(ixquad> VarBds(:,2)))
    ixquadfn = nan;
else
...
function [ncalls, xbest, xbestfn, xbestFnVar, xbestconstraint, ...
    ...
    constraint, ConstraintCov, mk, iseed, sseed, logfid, minmax,VarBds)
...
for j=0:jmax
    ...
    [x1, sseed]=PERTURB(xbest, sseed, logfid,VarBds);
    ...
    [fbar, gamma, npoints, plixbest, plixbestfn, plixbestFnVar, ...
        ...
        x1, mk, iseed, logfid, minmax,VarBds);
...
%% PERTURB
function [xpert, sseed] = PERTURB(x, sseed, logfid,VarBds)
...
xpert = checkCons(VarBds,xpert,x)';
end
...
% check and modify (if needed) the new point, based on VarBds.
    function modiSsolsV = checkCons(VarBds,ssolsV,ssolsV2) 
        col = size(ssolsV,2);
        ssolsV = reshape(ssolsV,1,length(ssolsV));
        ssolsV2 = reshape(ssolsV2,1,length(ssolsV2));
