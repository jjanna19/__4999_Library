...
function FMatrix = SIMOPT_multiSolvers_cons(problemnumber, solverArray, problemseed, solverseed, budgetV, logfilename, plotopt)
%initialize
...
problemhandle = str2func(problemname);
    ...
    for i=1:length(solverArray)
        ...
        for j=1:repsAlg  %do 30 replications for each algo on the problem
            [minmax, ~, ~, ~, VarBds, ~, ~, x0, ~, ~, ~] = problemstructhandle(NumStartingSols, problemseed);
            [~, soln, ~, ~, ~, ~, ~, ~, ~, ~] = ...
